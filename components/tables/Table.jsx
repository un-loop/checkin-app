...
const renderCell = (col) => (cell, cellIndex) => {
  const contents =typeof cell == 'function' ?
      cell(cellIndex) : cell;
  ...
  <TableRow key={rowIndex}>
    {
      row(rowData, rowIndex).map( (cell, cellIndex) =>
        renderCell(cols ? cols[cellIndex] : {})(cell, cellIndex))}
  </TableRow>;
...
export default (row, header, noData) => {
  ...
  class DecoratedTable extends React.Component {
    constructor(props) {
      ...
      this.state = {
        ...
        sortAscending: this.props.sortAscending && true,
      };
    ...
    handleChangeRowsPerPage(e) {
      ...
      this.props.onDataChange &&
        this.props.onDataChange(this.state.page, e.target.value,
    ...
    handleChangePage(e, page) {
      ...
      this.props.onDataChange &&
        this.props.onDataChange(e.target.value, this.state.rowsPerPage,
    ...
    getDataSlice() {
      ...
      if (this.state.sortField) {
        data = stableSort(data, this.state.sortField,
            dataCompare(this.state.sortAscending));
      } else if (!this.state.sortAscending) {
        data = data.map((i) => i).reverse();
      }

      return data.slice((this.state.page) * this.state.rowsPerPage,
    ...
    render() {
      ...
      return (
        <div className={classes.layout}>
          {this.props.title &&
              <Typography
                lassName={classes.title} variant='h6' color='inherit' noWrap>
                {this.props.title}
              </Typography>
          }
          <Table>
            {cols &&
                <TableHead>
                  <TableRow className={classes.header}>
                    {
                      cols.map(
                          this.renderHeader(this.getSortChangeHandler,
                              classes.sortHeader)
                      )
                    }
                  </TableRow>
          ...
          {!this.props.loading &&
            (!data || !data.length) && noData(classes.noData)
          }
          {this.props.loading &&
  ...
  DecoratedTable.propTypes = {
    ...
    classes: PropTypes.shape({
      ...
      sortHeader: PropTypes.string.isRequired,
    }).isRequired,
    title: PropTypes.string.isRequired,
  };
