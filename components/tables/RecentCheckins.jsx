...
const styles = (theme) => ({
  ...
  item: {
    ...
    '&:nth-child(4)': {
      WebkitMaskImage:
        '-webkit-gradient(linear, left top, left bottom, from(rgba(0,0,0,1)), to(rgba(0,0,0,.5)))', // eslint-disable-line max-len
    },
    '&:nth-child(5)': {
      WebkitMaskImage:
        '-webkit-gradient(linear, left top, left bottom, from(rgba(0,0,0,.5)), to(rgba(0,0,0,0)))', // eslint-disable-line max-len
    },
  },
  chip: {
    [theme.breakpoints.down('sm')]: {
          ...
          <GridList cols={1} className={classes.gridList}>
            {this.props.checkins.map((object, i) => (
              <GridListTile
                ...
                key={object.name + object.time}
                style={{height: 'auto', padding: 'auto'}}>
                <span>{object.name}</span>
                <span color='primary' className={classes.span}>
                  {timeAgo(object.checkin)}
                </span>
