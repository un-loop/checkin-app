...
class EventLanding extends React.Component {
    ...
    return axios
        ...
        .then((response) => {
          const records = (response.data)
              .map( ({name, start, eventId, started}) =>
                ({name, start, eventId, started}));
  ...
  async saveEvent(detail) {
    ...
    action.then(
        ...
        (err) => {
          this.setState({
            saveError: {message: err.message, errorKey: Date.now()},
            isSaving: false});
        }
    );
  }
...
wrapped.displayName = 'EventLanding';
