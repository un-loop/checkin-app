...
class Checkin extends React.Component {
  constructor(props) {
    ...
    this.state =
      {loading: true, attendee: '', event: {name: ''}, isCheckin: false};
  ...
  refreshLatest() {
    ...
    return axios
        .get(
            ...
            + `/attendees/?top=${this.props.showLast}`
  ...
  async saveCheckin(detail) {
    ...
    try {
      ...
      this.setState(
          {attendee: '', isCheckin: false, isSaving: false, event: event}
      );

      this.refreshLatest(); // don't need to await
    } catch (ex) {
      this.setState(
          {saveError: {message: ex.message, errorKey: Date.now()},
            isSaving: false}
      );
    }
  }
  ...
  render() {
    return (
      <Page title={this.state.event.name}>
        ...
        {
          (this.state.loading ||
            (this.state.latestAttendees
              && this.state.latestAttendees.length !== 0)
          )
            &&
            <RecentCheckins
